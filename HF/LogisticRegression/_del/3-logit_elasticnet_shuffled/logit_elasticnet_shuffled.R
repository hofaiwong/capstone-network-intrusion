...
lambda = exp(-2) #lamdba.min still keeps ~100+ features. Need to balance complexity and accuracy
...
logit.train.class.final = predict(logit.cv, 
                                  s = lambda, 
                                  type = 'class',
                                  newx = x.train)
performance(y.train, logit.train.class.final) 
...
logit.coef = predict(logit.cv, 
                     s = lambda, 
                     type = 'coefficients')
sum(logit.coef!=0)-1 #Keep ~115 features with lambda.min, 15 with lambda=exp(-4)
logit.coef
